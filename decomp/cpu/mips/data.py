...
def get_arg_for_va_function(callee, start_ea):
    ...
    while True:
        if distance > 10:
            raise VarargsError('gave up looking for needed varargs argument for %s between %s..%s' % (ida.atoa(ea), ida.atoa(start_ea)))

        if ea == start_ea:
            ea = ida.prev_head(ea)
            continue  # skip the call insn
        elif list(ida.code_refs_from(ea, 0)) != []:
            raise VarargsError('encountered branch/jump while looking for varargs argument between %s..%s' % (ida.atoa(ea), ida.atoa(start_ea)))
...
def get_swval(start_ea):
    ...
    while True:
        if distance > 10:
            raise SwitchTroubleError('gave up looking for switch value between %s..%s' % (ida.atoa(ea), ida.atoa(start_ea)))
        ...
        if ida.is_switch_insn(ea):
            ...
            raise SwitchTroubleError('encountered branch/jump while looking for switch value between %s..%s' % (ida.atoa(ea), ida.atoa(start_ea)))
